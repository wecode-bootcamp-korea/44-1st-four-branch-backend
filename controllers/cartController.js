...
const deleteFromCart = catchAsync(async (req, res) => {
  ...
  const carts = await cartService.deleteFromCart(cartId, req.user.id);

  res.status(200).json(carts);
});

const readCart = catchAsync(async (req, res) => {
  const carts = await cartService.readCart(req.user.id);
  res.status(200).json(carts);
});

const changeQuantity = catchAsync(async (req, res) => {
  const { cartId, quantity } = req.body;
  const carts = await cartService.changeQuantity(cartId, quantity, req.user.id);
